...
int ctpp(uint64_t *evset, int evset_max, uint64_t victim, int nway, uint64_t page, int* evset_len);
...
void attacker(){
  ...
  for (uint64_t t=0; t<TEST_LEN; t++) {
    ...
    if(ctpp(evset, 32, target_addr, LLC_WAYS, (uint64_t)evict_mem, &evset_len)) {
      succ++;
      char disp = 0;
      if(succ == 1) disp = 1;
